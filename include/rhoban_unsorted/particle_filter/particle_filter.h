...
protected:
   ...
   * 1 = every particle)
   */
  virtual void apply(const Observation<P>& obs, double weight = 1.0, double ratioUsed = 1.0)
  {
    if (ratioUsed != 1.0)
    {
...
public:
  ...
  void initializeAtUniformRandom(double minVal, double maxVal, int particleNb)
  {
  ...
  void initializeAtUniformRandom(double minVal, double maxVal, int particleNb, const Eigen::VectorXd& center)
  {
    clear();
  ...
  void partialUniformResampling(double resampledRatio, double* minValues, double* maxValues)
  {
    int particleNb = particles.size() * resampledRatio;
    std::vector<size_t> particles_index;
    particles_index = rhoban_random::getKDistinctFromN(particleNb, particles.size(), &engine);
    Eigen::MatrixXd limits = getLimits(minValues, maxValues);
    for (int k : particles_index)
    {
      P p;
      // Generate a new particle using the limits
      p.setFromVector(rhoban_random::getUniformSamples(limits, 1, &engine)[0]);
      particles[k] = std::pair<P, double>(p, 1.0);
    }
  }

  // Allows to initialize the particle
  void initializeAtUniformRandom(double* minValues, double* maxValues, int particleNb)
  {
    clear();
    Eigen::MatrixXd limits = getLimits(minValues, maxValues);
    for (int k = 0; k < particleNb; k++)
  ...
  virtual void mutate(Controller<P>& ctrl, double elapsedTime)
  {
    if (particles.size() == 0)
    {
      return;
  ...
  virtual void step(Controller<P>& ctrl, double elapsedTime)
  {
  ...
  /* Perform a full step of the ParticleFilter, usually a mutation followed
     by a selection */
  virtual void step(Controller<P>& ctrl, const Observation<P>& obs, double elapsedTime)
  {
    mutate(ctrl, elapsedTime);
    apply(obs);
    select();
   ...
   */
  virtual void step(Controller<P>& ctrl, const std::vector<Observation<P>*>& observations, double elapsedTime)
  {
    mutate(ctrl, elapsedTime);
  ...
  void resize(size_t new_nb_particles)
  {
    ...
    {
      std::vector<size_t> particles_index;
      particles_index = rhoban_random::getKDistinctFromN(new_nb_particles, particles.size(), &engine);
