...
public:
  ...
  static double linearScore(double error, double maxError, double pFalsePositive)
  {
   ...
   */
  static double quadraticScore(double error, double maxError, double pFalsePositive)
  {
    double ratioError = error / maxError;
    double score = 1 - ratioError;
    score = score > 0 ? score : 0.0;
   ...
   */
  static double sigmoidScore(double error, double maxError, double pFalsePositive, double offset, double lambda)
  {
    double ratioError = error / maxError;
    double x, score;
