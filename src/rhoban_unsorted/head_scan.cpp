...
{
  if (fov_width == new_fov_width && fov_height == new_fov_height)
  {
    return;
  }
  dirty = true;
...
{
  if (new_min_overlap == min_overlap)
    return;
  dirty = true;
...
{
  if (max_speed == ctrl_law.max_vel && max_acc == ctrl_law.max_acc)
  {
    return;
  }
  dirty = true;
...
void HeadScan::updateControlPoints()
{
  ...
  for (int row = 0; row < nb_rows; row++)
  {
    ...
    {
      tilt = tilt_end - (row - 1) * tilt_offset;
    }
    // Computing pan
...
HeadScan::PanTilt HeadScan::getTarget(double time) const
{
  ...
  if (isOddCycle(time) && getNbRows() % 2 == 1)
  {
    target(0) *= -1;
  }
  return target;
