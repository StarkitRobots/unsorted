...
{
LogModel::LogModel() : _isLogging(false), _data(), _container(), _scheduling()
{
}
...
void LogModel::stop(const std::string& path)
{
  ...
  {
    _isLogging = false;
    std::string filename = std::string(path + "model_") + Leph::currentDate() + std::string(".log");
...
void LogModel::logModel(Leph::HumanoidFixedModel& model)
{
  ...
  Eigen::Vector3d deltaLeftFoot = model.get().position("left_foot_tip", "right_foot_tip");
  Eigen::Vector3d deltaRightFoot = model.get().position("right_foot_tip", "left_foot_tip");
  ...
  _data.setOrAppend("model:is_support_left", (model.getSupportFoot() == Leph::HumanoidFixedModel::LeftSupportFoot));
